...
- [x] 1.1 Add `withPosition(route, key, id)` to `src/toc.js` — appends `?<key>=<encoded id>`
...
- [x] 1.2 Have `routeFrom` in `src/api.js` split the position off the hash on the first `?`
      ...
      slash.
- [x] 1.3 Point `linkedHeading` (`src/toc.js`) and `linkedScenario` (`src/spec.js`) at a
...
- [x] 1.4 Extend `test/route.test.mjs`: a route carrying a position parses to the same
...
- [x] 1.5 Extend `test/toc.test.mjs` and `test/spec.test.mjs` for the new shapes, and add
...
- [x] 1.6 Verify: `pnpm test` — the toc, route and spec suites are the change at this point;
      the app is expected to be mid-move and is not run yet.

## 2. The pages that read a position

- [x] 2.1 `src/components/ScenarioRef.jsx` — `target()` builds `#…?at=<id>` through
...
- [x] 2.2 `src/components/CopyLink.jsx` — take the position rather than a pre-built query
      string, and rewrite the paragraph explaining why it was a query.
- [x] 2.3 `src/components/WithOutline.jsx` — the rail's click handler pushes
...
- [x] 2.4 `src/components/SpecText.jsx` — `linkedScenario()` now reads the hash; the
      `useMemo(…, [])` that pins it to arrival stays exactly as it is.
- [x] 2.5 `src/views/ChangeDetail.jsx` and `src/views/Catalog.jsx` — `tabAsked` and the
...
- [x] 2.6 Verify: `pnpm test && pnpm build`, then in the browser copy a heading link and a
      scenario link, paste each into a new tab, and confirm the page opens on the right tab,
      scrolled and marked.

## 3. The scrub comes out

- [x] 3.1 Delete the stale-query branch of `useRoute` in `src/api.js`: the `opened` ref, the
...
- [x] 3.2 Keep the route object when only the position changed: compare `view`/`arg`/`tab`
...
- [x] 3.3 Verify: `pnpm test && pnpm build`, then walk it in the browser — open a heading
...
- [x] 4.1 Add the one-time rewrite to `src/main.jsx`, before `createRoot`: a `to` or `at` in
...
- [x] 4.2 Test it in `test/route.test.mjs` against the scenarios in the spec: the plain old
      ...
      equivalent.
- [x] 4.3 Verify: `pnpm test && pnpm build`, then paste an old-shape link into the browser —
      it opens on the position and the address bar shows the new shape, and one press of back
      leaves the viewer.

## 5. The docs

- [x] 5.1 `README.md` — rewrite "A position leaves with the page it named" around the new
      ...
      query.
- [x] 5.2 `CONTEXT.md` — add **position** as a term if it is not already one: a heading or a
...
- [x] 5.3 Verify: `pnpm test && pnpm build`, and read the README passages against the
