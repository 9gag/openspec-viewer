...
grouped so a reader can find one by name, and marked so a capability two changes in
development are both rewriting cannot be mistaken for a quiet one.
...
#### Scenario: A capability with a baseline and no in-development change
...
#### Scenario: A capability only an in-development change names

- **GIVEN** an in-development change deltaing `storefront/membership`, which has no baseline
- **WHEN** the catalog is read
...
- **GIVEN** a capability that only an in-development change deltas
- **WHEN** its row is read
...
- **GIVEN** a capability with a REMOVED delta in an in-development change and a baseline
  still in `openspec/specs/`
- **WHEN** its row is read
...
A capability that two or more in-development changes delta is **contested**: at archive
time the second change is written against a baseline the first already rewrote, and
nothing else in the store says so. Each row SHALL state how many in-development changes
are deltaing it when there is at least one, and a contested row SHALL be distinguishable
from a row with a single one without reading the number.

#### Scenario: One change in development

- **GIVEN** `storefront/product-page` deltaed by exactly one in-development change
- **WHEN** its row is read
- **THEN** the row says the capability is in development

#### Scenario: Two changes in development

- **GIVEN** `storefront/pricing` deltaed by two in-development changes
- **WHEN** its row is read
- **THEN** the row says two changes are in development, and is marked as contested rather
  than as one change in development

#### Scenario: Archived changes do not contest

- **GIVEN** a capability deltaed by four archived changes and no in-development change
- **WHEN** its row is read
- **THEN** the row is not marked, and states no in-development count
...
- **GIVEN** a store where no capability has more than one in-development change
- **WHEN** the catalog is read
...
- **THEN** `spec/<id>` shows every change that touched it, in development and archived,
  newest first
